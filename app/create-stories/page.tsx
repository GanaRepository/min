...
export default function CreateStoriesPage() {
  ...
  const handleCreateStory = async () => {
    ...
    try {
      ...
      if (status === 'unauthenticated' || !session) {
        console.log(
          '❌ User not authenticated, storing elements and redirecting to login'
        );
        try {
      ...
      try {
        ...
        limitsData = { error: text };
      }
      if (limitsResponse.ok) {
        if (!limitsData.canCreateStory) {
          setToastMessage(
            limitsData.message || "You've reached your monthly story limit."
          );
          setToastType('destructive');
          if (limitsData.upgradePrompt) {
            setToastMessage(limitsData.upgradePrompt);
            setToastType('default');
          }
          setIsCreating(false);
          return;
        } else if (typeof limitsData.remaining !== 'undefined') {
          setToastMessage(
            `You have ${limitsData.remaining} stories left this month.`
          );
          setToastType('default');
        }
      } else {
        setToastMessage(
          limitsData.error || 'Unable to check subscription limits.'
        );
        setToastType('destructive');
        setIsCreating(false);
        return;
  ...
  const loadPendingElementsAndCreateStory = async (token: string) => {
    try {
      ...
      const response = await fetch(
        `/api/stories/pending-elements?token=${token}`
      );
      ...
      // Validate elements
      const incompleteElements = Object.entries(data.elements).filter(
        ([_, value]) => !value
      );
      if (incompleteElements.length > 0) {
        toast({
      ...
      router.replace(
        `/children-dashboard/story/${createData.session.storyNumber || createData.session.id}`
      );
    } catch (error) {
  ...
  const isCurrentStepComplete = selectedElements[elementType as string] !== '';
          ...
          <main className="flex-1 p-4 sm:p-6 mt-8 mb-8">
            <div className="max-w-7xl mx-auto">
              ...
              >
                ...
                <motion.button
                  whileHover={
                    !isCreating && isCurrentStepComplete ? { x: 2 } : {}
                  }
                  whileTap={
                    !isCreating && isCurrentStepComplete ? { scale: 0.98 } : {}
                  }
                  onClick={
