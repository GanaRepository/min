  ...
  const handleCreateStory = async () => {
    ...
    try {
      ...
      if (incompleteElements.length > 0) {
        setToastMessage('Please select all story elements first!');
        setToastType('destructive');
        setIsCreating(false);
        return;
      }
      setToastMessage(
        'Getting everything ready for your creative adventure...'
      );
      setToastType('default');
      if (status === 'unauthenticated' || !session) {
        // Store elements in backend and redirect to login with callbackUrl
        const storeResponse = await fetch(
          '/api/stories/store-pending-elements',
          {
            method: 'POST',
            headers: { 'Content-Type': 'application/json' },
        ...
        if (!storeResponse.ok || !storeData.token) {
          console.error('Store-pending-elements error:', storeData.error);
          setToastMessage(storeData.error || 'Failed to store elements');
          setToastType('destructive');
          setIsCreating(false);
          return;
        }
      ...
      if (session.user.role !== 'child') {
        setToastMessage('Only children can create stories.');
        setToastType('destructive');
        setIsCreating(false);
        return;
      }
      // Authenticated: create story session
      ...
      if (!response.ok || !data.session) {
        ...
        setToastMessage(data.error || 'Failed to create story session');
        setToastType('destructive');
        setIsCreating(false);
        return;
      }
      setToastMessage('Your magical adventure begins now!');
      setToastType('default');
      if (typeof window !== 'undefined') {
        localStorage.removeItem('pendingStoryElements');
      }
      router.push(
        `/children-dashboard/story/${data.session.storyNumber || data.session.id}`
      );
    } catch (error) {
    ...
    if (
      ...
      isProcessingPendingToken ||
      (pendingToken && !hasLoadedFromStorage)
    ) {
      return (
          ...
          <div className="text-white text-center">
            ...
            <p>
              ...
              (pendingToken && !hasLoadedFromStorage)
                ...
                : 'Loading story creation...'}
            </p>
            {(isProcessingPendingToken ||
              (pendingToken && !hasLoadedFromStorage)) && (
              <p className="text-sm text-gray-400 mt-2">
                This may take a moment while we prepare everything!
              </p>
            )}
          </div>
        </div>
            ...
            <div className="max-w-7xl mx-auto w-full">
              <div className="grid grid-cols-1 lg:grid-cols-2 gap-12 items-center">
                ...
                >
                  ...
                  >
                    Follow our simple 6-step process to transform your
                    imagination into incredible stories with AI collaboration
                    ...
                    <Link href="/contact-us">
                      <motion.button
                        ...
                        whileHover={{
                          scale: 1.05,
                          boxShadow:
                            '0 20px 40px -10px rgba(147, 51, 234, 0.6)',
                        }}
                        whileTap={{ scale: 0.95 }}
                      >
            ...
            <div className="max-w-6xl mx-auto">
              ...
              >
                ...
                <p className="text-xl text-gray-300 max-w-3xl mx-auto">
                  Our AI-powered platform makes story creation fun and
                  educational through collaborative writing
                </p>
              </motion.div>
            ...
            <div className="max-w-4xl mx-auto text-center">
              ...
              >
                ...
                <p className="text-xl text-gray-300 mb-8 max-w-2xl mx-auto">
                  Join thousands of young writers who are already creating
                  amazing stories with AI assistance
                </p>
                ...
                <AnimatePresence mode="wait">
                  ...
                  >
                    {currentElements.map((element: any, index: number) => {
                      const isSelected =
                        selectedElements[elementType as string] ===
                        element.name;
                    ...
                    {allStepsComplete && (
                      ...
                      >
                        ...
                        <div className="text-green-300 text-sm">
                          ...
                          <strong>{selectedElements.genre}</strong> adventure
                          with <strong>{selectedElements.character}</strong> in{' '}
