...
export default function EditCreativeContest() {
    ...
    try {
      ...
      if (data.success) {
        ...
        setFormData({
          ...
          resultsDate: new Date(contest.resultsDate).toISOString().slice(0, 16),
          acceptedFormats: contest.acceptedFormats || [],
          maxFileSize: contest.maxFileSize,
          maxSubmissionsPerUser: contest.maxSubmissionsPerUser,
          rules: contest.rules,
          prizes: contest.prizes || [],
          showPrizes: contest.showPrizes,
  ...
  useEffect(() => {
    ...
    fetchContestData();
  }, [session, router, contestId, fetchContestData]);
  ...
  const handleSubmit = async () => {
    ...
    try {
      ...
      if (data.success) {
        router.push(`/admin/creative-contests/${contestId}`);
      } else {
        setErrors({ submit: data.error || 'Failed to update contest' });
      }
    } catch (error) {
      setErrors({ submit: 'Failed to update contest' });
    } finally {
      setSaving(false);
    }
  };
  ...
  return (
    <div className="min-h-screen bg-gray-900">
      <div className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="mb-8">
          <button
            onClick={() => router.push(`/admin/creative-contests/${contestId}`)}
                  ...
                  {formData.prizes.map((prize, index) => (
                    ...
                    >
                      ...
                      <button
                        ...
                        className="text-red-400 hover:text-red-300 p-2 rounded transition-colors flex items-center"
                      >
                        <Minus size={16} />
