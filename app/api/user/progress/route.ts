...
export async function GET(request: NextRequest) {
  try {
    ...
    console.log('🎯 Found scores for', allScores.length, 'completed stories:', allScores);
    ...
    if (allScores.length > 0) {
      ...
      averageScore = Math.round(allScores.reduce((sum, score) => sum + score.overall, 0) / allScores.length);
    }
    ...
    const periodAverageScore = periodScores.length > 0 ? 
      Math.round(periodScores.reduce((a, b) => a + b, 0) / periodScores.length) : 0;
