...
export async function GET() {
  try {
   ...
   const totalWords = completedSessions.reduce((sum, session) => sum + (session.childWords || 0), 0);
   ...
   const averageScore = completedSessions.length > 0 
     ...
     : 0;
   const bestScore = Math.max(...completedSessions.map(session => session.overallScore || 0), 0);
   ...
   const weeklyStats = await StorySession.aggregate([
     ...
     },
     {
       $group: {
         ...
         stories: { $sum: { $cond: [{ $eq: ['$status', 'completed'] }, 1, 0] } },
   ...
   const skills = {
     grammar: {
       current: Math.min(averageScore + Math.floor(Math.random() * 10) - 5, 100),
       improvement: Math.floor(Math.random() * 5) + 1
     },
     creativity: {
       current: Math.min(averageScore + Math.floor(Math.random() * 15) - 5, 100),
       improvement: Math.floor(Math.random() * 7) + 1
     },
     vocabulary: {
       current: Math.min(averageScore + Math.floor(Math.random() * 12) - 3, 100),
       improvement: Math.floor(Math.random() * 4) + 1
     },
     storytelling: {
       current: Math.min(averageScore + Math.floor(Math.random() * 8) - 2, 100),
