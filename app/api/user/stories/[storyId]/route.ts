...
// app/api/user/stories/[storyId]/route.ts - FIXED TYPESCRIPT ERRORS
import { NextResponse } from 'next/server';
import { connectToDatabase } from '@/utils/db';
import StorySession from '@/models/StorySession';
import Turn from '@/models/Turn';
...
// GET endpoint for fetching a single story
export async function GET(
  request: Request,
  { params }: { params: { storyId: string } }
) {
    ...
    const formattedStory = {
      ...
      assessment: story.assessment
        ? {
            ...
            overallScore: story.assessment.overallScore || 0,
            creativity:
              story.assessment.categoryScores?.creativity ||
              story.assessment.creativityScore ||
              0,
            grammar:
              story.assessment.categoryScores?.grammar ||
              story.assessment.grammarScore ||
              0,
            vocabulary:
              story.assessment.categoryScores?.vocabulary ||
              story.assessment.vocabularyScore ||
              0,
            structure:
              story.assessment.categoryScores?.structure ||
            ...
            feedback:
              story.assessment.educationalFeedback?.teacherComment ||
              story.assessment.feedback ||
              '',
            strengths:
              story.assessment.educationalFeedback?.strengths ||
              story.assessment.strengths ||
              [],
            improvements:
              story.assessment.educationalFeedback?.improvements ||
              story.assessment.improvements ||
              [],
            integrityRisk:
...
// DELETE endpoint - REMOVED FOR CHILDREN'S SAFETY
// Children can no longer delete their stories to prevent accidental loss
export async function DELETE(
  request: Request,
  { params }: { params: { storyId: string } }
) {
