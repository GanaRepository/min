...
export async function GET(request: NextRequest, { params }: { params: { id: string } }) {
  try {
    ...
    const story = await StorySession.findById(storyId).populate('childId', 'firstName lastName email');
    if (!story) {
      return NextResponse.json({ success: false, error: 'Story not found' }, { status: 404 });
    }
    // Check if the child is assigned to this mentor
    const assignment = await MentorAssignment.findOne({ mentorId, childId: story.childId._id, isActive: { $ne: false } });
    if (!assignment) {
      return NextResponse.json({ success: false, error: 'Not authorized for this story' }, { status: 404 });
    }
