...
// app/api/stories/export/[storyId]/[format]/route.ts - Updated for 42-Factor Assessment
...
import Turn from '@/models/Turn';
import mongoose from 'mongoose';
import { ComprehensivePDFGenerator } from '@/lib/export/pdf-generator';
import { ComprehensiveWordGenerator } from '@/lib/export/word-generator';
...
export async function GET(
  ...
  try {
    ...
    // Prepare comprehensive story data with 42-factor assessment
    const comprehensiveStoryData = {
      title: storySession.title,
      content: storyContent,
      totalWords: storySession.totalWords || storyContent.split(/\s+/).filter(Boolean).length,
      authorName: `${session.user.firstName} ${session.user.lastName}`,
      publishedAt: storySession.updatedAt.toISOString(),
      assessment: storySession.assessment?.assessmentVersion === '3.0-42-factor' 
        ? {
            overallScore: storySession.assessment.overallScore,
            status: storySession.assessment.status,
            statusMessage: storySession.assessment.statusMessage,
            allFactors: storySession.assessment.allFactors,
            aiDetectionResult: storySession.assessment.aiDetectionResult,
          }
        : {
            // Legacy assessment fallback
            overallScore: storySession.overallScore || storySession.assessment?.overallScore || 0,
            status: 'Legacy Assessment',
            statusMessage: 'This story uses an older assessment format',
          }
    };
    ...
    if (format === 'pdf') {
      const pdfGenerator = new ComprehensivePDFGenerator();
      fileBlob = pdfGenerator.generateStoryPDF(comprehensiveStoryData);
      filename = `${storySession.title}_42-factor-assessment.pdf`;
      contentType = 'application/pdf';
    } else {
      const wordGenerator = new ComprehensiveWordGenerator();
      fileBlob = await wordGenerator.generateStoryDocument(comprehensiveStoryData);
      filename = `${storySession.title}_42-factor-assessment.docx`;
    ...
    console.error('Error exporting 42-factor story:', error);
    return NextResponse.json(
      { error: 'Failed to export story with comprehensive assessment' },
      { status: 500 }
    );
  }
}