...
export async function POST(
  ...
  console.log('🎯 ADVANCED AI ASSESSMENT API called for session:', params.sessionId);
  ...
  try {
    ...
    console.log(`📖 Starting ADVANCED assessment with AI detection for age: ${userAge}`);
    ...
    if (storySession.type === 'collaborative') {
      ...
      console.log(`📝 Collaborative story: ${userContributions.length} user turns`);
    } else {
      fullStoryContent = storySession.storyContent || storySession.content || '';
    ...
    if (!fullStoryContent || fullStoryContent.trim().length < 50) {
      return NextResponse.json(
        { error: 'Story content too short for meaningful assessment (minimum 50 words)' },
        { status: 400 }
      );
    }

    console.log('🤖 Starting comprehensive AI-powered assessment with advanced AI detection...');

    // PERFORM COMPREHENSIVE ASSESSMENT WITH ADVANCED AI DETECTION
    const comprehensiveAssessment = await ComprehensiveAssessmentEngine.performCompleteAssessment(
      fullStoryContent,
      {
        childAge: userAge,
    ...
    console.log(`🔒 Integrity: ${comprehensiveAssessment.integrityAnalysis.overallStatus}`);
    console.log(`🤖 AI Detection: ${comprehensiveAssessment.integrityAnalysis.aiDetection.aiLikelihood}`);
    console.log(`📝 Human-like Score: ${comprehensiveAssessment.integrityAnalysis.aiDetection.humanLikeScore}%`);
    ...
    const assessmentData = {
     ...
     characterDevelopmentScore: comprehensiveAssessment.storyDevelopment.characterDevelopment.score,
     plotDevelopmentScore: comprehensiveAssessment.storyDevelopment.plotDevelopment.score,
     ...
     improvements: comprehensiveAssessment.comprehensiveFeedback.areasForEnhancement,
     ...
     integrityAnalysis: {
       ...
       aiDetection: {
         humanLikeScore: comprehensiveAssessment.integrityAnalysis.aiDetection.humanLikeScore,
         aiLikelihood: comprehensiveAssessment.integrityAnalysis.aiDetection.aiLikelihood,
         confidenceLevel: comprehensiveAssessment.integrityAnalysis.aiDetection.confidenceLevel,
         analysis: comprehensiveAssessment.integrityAnalysis.aiDetection.analysis,
         riskLevel: comprehensiveAssessment.integrityAnalysis.aiDetection.riskLevel,
         indicators: comprehensiveAssessment.integrityAnalysis.aiDetection.indicators,
       },
       // Plagiarism Results
       plagiarismCheck: {
         originalityScore: comprehensiveAssessment.integrityAnalysis.plagiarismCheck.originalityScore,
         riskLevel: comprehensiveAssessment.integrityAnalysis.plagiarismCheck.riskLevel,
         violations: comprehensiveAssessment.integrityAnalysis.plagiarismCheck.violations,
         status: comprehensiveAssessment.integrityAnalysis.plagiarismCheck.status,
       },
       ...
       recommendation: comprehensiveAssessment.integrityAnalysis.recommendation,
     },
   ...
   } else if (comprehensiveAssessment.integrityAnalysis.overallStatus === 'WARNING') {
   ...
   const updateData = {
     ...
     aiDetectionScore: comprehensiveAssessment.integrityAnalysis.aiDetection.humanLikeScore,
     plagiarismScore: comprehensiveAssessment.integrityAnalysis.plagiarismCheck.originalityScore,
     
     // Flag for admin review if necessary
     ...(storyStatus === 'flagged' && {
       adminFlags: {
         aiDetected: comprehensiveAssessment.integrityAnalysis.aiDetection.riskLevel === 'CRITICAL RISK',
         plagiarismDetected: comprehensiveAssessment.integrityAnalysis.plagiarismCheck.status === 'VIOLATION',
         flaggedAt: new Date(),
   ...
   if (storyStatus === 'flagged') {
     ...
     console.log(`   🤖 AI Detection: ${comprehensiveAssessment.integrityAnalysis.aiDetection.aiLikelihood} (${comprehensiveAssessment.integrityAnalysis.aiDetection.humanLikeScore}% human-like)`);
     console.log(`   📝 Plagiarism: ${comprehensiveAssessment.integrityAnalysis.plagiarismCheck.riskLevel} risk`);
     console.log(`   ⚠️  Status: ${comprehensiveAssessment.integrityAnalysis.overallStatus}`);
     console.log(`   💬 Message: ${comprehensiveAssessment.integrityAnalysis.message}`);
   } else {
   ...
   return NextResponse.json({
     success: true,
     assessment: assessmentData,
     message: 'Advanced AI-powered assessment with integrity verification completed',
     session: {
     ...
     integrityCheck: {
       ...
       aiDetection: comprehensiveAssessment.integrityAnalysis.aiDetection.aiLikelihood,
       humanLikeScore: comprehensiveAssessment.integrityAnalysis.aiDetection.humanLikeScore
     }
   });
   ...
   return NextResponse.json(
     { 
       ...
       details: 'Unable to complete AI-powered assessment with integrity verification.',
