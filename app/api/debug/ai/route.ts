...
export async function GET() {
  try {
    ...
    return NextResponse.json({
      ...
      quickFix: {
        message:
          'Check the server console for detailed logs with [AI-DIAGNOSTICS] prefix',
        envVarsNeeded: [
...
export async function POST(request: Request) {
  try {
    const {
      prompt = 'Test: Generate a single sentence about a brave knight.',
    } = await request.json();
    ...
    const isTemplate =
      response.includes('What an exciting') ||
