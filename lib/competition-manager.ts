  ...
  export class CompetitionManager {
    ...
    for (let i = 0; i < stories.length; i++) {
      ...
      try {
        // Run the same teacher-style 13-factor assessment
        ...
        // Simple numeric proxy for ranking → length of positive feedback
        const feedbackText = JSON.stringify(assessment);
        const proxyScore = Math.min(
          100,
          Math.round(feedbackText.split(/\s+/).length / 10) // 1 point per ~10 words
        );
        ...
        finalResults.push({
          ...
          proxyScore,
          fullAnalysis: assessment, // ✅ Store teacher-style 13-factor feedback
        });

        console.log(`   ✅ "${story.title}": proxyScore ${proxyScore}`);
      } catch (error) {
        ...
        finalResults.push({
          ...
          childEmail: story.childId.email,
          proxyScore: 0,
          fullAnalysis: { error: "Comprehensive analysis failed" },
        });
      }
    }

    // Pick top 3 winners based on proxyScore
    ...
    console.log(`🏆 [WINNERS] Final 3 selected with teacher-style analysis`);
    winners.forEach((w) =>
      console.log(`   ${w.place}: ${w.childName} - "${w.title}"`)
    );
    ...
    for (let i = 0; i < stories.length; i++) {
      ...
      try {
        ...
        const proxyScore = Math.min(
          100,
          Math.round(feedbackText.split(/\s+/).length / 10)
        );
        results.push({
          storyId: story._id,
          title: story.title,
        ...
        results.push({
          ...
          childEmail: story.childId.email,
          proxyScore: 0,
          fullAnalysis: { error: "Analysis failed" },
        });
      }
    }
    ...
    static async lightweightAIAnalysis(story: any) {
      ...
      try {
        // Use teacher-style assessment for lightweight analysis (proxy score)
        const assessment = await SingleCallAssessmentEngine.performAssessment(
          lightPrompt,
          {
            childAge: 10,
        ...
        const feedbackText = JSON.stringify(assessment);
        const score = Math.min(95, Math.max(60, Math.round(feedbackText.split(/\s+/).length / 10)));
        return {
          score,
