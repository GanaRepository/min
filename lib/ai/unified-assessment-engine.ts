...
export class UnifiedAssessmentEngine {
  ...
  private static detectSubplot(story: string): boolean {
    ...
    const characters = this.identifyCharacters(story);
    const conflicts = story
      .toLowerCase()
      .match(/\b(problem|trouble|conflict)\b/g);
  ...
  private static detectLogicalInconsistencies(story: string): string[] {
    ...
    if (
      ...
      content.includes('magically')
    ) {
      const sentences = story.split(/[.!?]+/);
      const magicIndex = sentences.findIndex((s) =>
        /magic|suddenly|magically/.test(s.toLowerCase())
      );
      if (magicIndex > 0 && magicIndex < sentences.length - 1) {
        const hasSetup = sentences
          .slice(0, magicIndex)
          .some((s) => /wizard|fairy|spell|power/.test(s.toLowerCase()));
        if (!hasSetup) {
  ...
  private static checkLogicalSequence(sentences: string[]): number {
    ...
    for (let i = 0; i < sentences.length - 1; i++) {
      ...
      if (current.includes('problem') && next.includes('solve'))
        logicalConnections++;
      if (current.includes('door') && next.includes('open'))
        logicalConnections++;
      if (current.includes('ran') && next.includes('tired'))
        logicalConnections++;
      if (current.includes('rain') && next.includes('wet'))
        logicalConnections++;
    }
  ...
  private static detectSophisticatedCausation(story: string): number {
    ...
    // Multiple causation
    if (content.includes('not only') && content.includes('but also'))
      bonus += 10;
  ...
  private static assessThemeDevelopment(
    story: string,
    themes: string[]
  ): number {
    let development = 0;
  ...
  private static assessTopicAppropriatenessForAge(
    content: string,
    age: number
  ): number {
    let score = 100;
