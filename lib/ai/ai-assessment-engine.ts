...
export class AIAssessmentEngine {
  ...
  static async assessStory(
    ...
    const assessmentResult = await this.performCompleteAssessment(storyContent, {
...
static async performLightweightAssessment(prompt: string): Promise<string> {
 ...
 static async checkPlagiarism(
   ...
   const { recommendations, educationalFeedback } = this.generatePlagiarismEducationalResponse(
     violations,
     overallScore,
     metadata
   );
 ...
 private static calculateIntegrityRisk(
   ...
   const plagiarismScore = { low: 10, medium: 30, high: 60, critical: 90 }[plagiarismRisk] || 10;
   ...
   if (aiLikelihood === 'very_high' || aiLikelihood === 'high') return 'critical';
 ...
 static getIntegrityAssessment(integrityAnalysis: any): {
   ...
   const { aiDetectionResult, integrityRisk, originalityScore } = integrityAnalysis;
   ...
   if (aiDetectionResult.likelihood === 'very_high' || aiDetectionResult.likelihood === 'high') {
     return {
       ...
       recommendation: 'REJECT - This appears to be written by artificial intelligence. Require original, authentic writing.'
     };
   }
   ...
   if (integrityRisk === 'critical') {
     return {
       ...
       recommendation: 'REJECT - Significant concerns about content authenticity.'
     };
   }
   ...
   if (integrityRisk === 'high' || originalityScore < 50) {
     return {
       ...
       recommendation: 'REVIEW - Content shows signs of potential AI assistance or copying.'
     };
   }
 ...
 private static generateComprehensiveFeedback(
   ...
   if (integrityAnalysis.aiDetectionResult.likelihood === 'very_high' || 
       integrityAnalysis.aiDetectionResult.likelihood === 'high') {
     
     improvements.unshift('This content appears to be AI-generated. Focus on authentic, original writing.');
     nextSteps.unshift('Write about your own experiences and use your natural voice.');
     
     // Override teacher comment for AI detection
     const teacherComment = age <= 8 
     ...
     const encouragement = "Remember, the best stories come from your own imagination and experiences. Keep practicing authentic writing!";
 ...
 private static getCategoryImprovementMessage(category: string, age: number): string {
   const messages = {
     ...
     sensoryDetails: 'Include what characters see, hear, smell, taste, and feel',
 ...
 private static generateNextSteps(improvements: string[], age: number): string[] {
   ...
   if (improvements.some((imp) => imp.includes('characters'))) {
     steps.push('Give your characters names, feelings, and interesting personalities');
   }
 ...
 private static generatePersonalizedRecommendations(
   ...
   if (educationalAssessment.categoryScores.characterDevelopment < 70) {
     ...
     practiceExercises.push('Write about a character who is the opposite of you');
   }

   if (educationalAssessment.categoryScores.plotDevelopment < 70) {
     immediate.push('Create more exciting conflicts and resolutions');
     practiceExercises.push('Write a story where the main character faces three challenges');
   }
 ...
 private static generatePlagiarismEducationalResponse(
   ...
   const educationalFeedback = this.generatePlagiarismEducationalFeedback(score, violations, ageGroup);

   return { recommendations, educationalFeedback };
 }

 private static generatePlagiarismEducationalFeedback(score: number, violations: any[], ageGroup: string): string {
   if (score >= 90) {
 ...
 private static getGrammarRulesForAge(ageGroup: string) {
   ...
   if (ageGroup === '6-8') {
     ...
     return baseRules.concat([
       { pattern: /\bwho's\b.*\bcar/gi, penalty: 7, description: "Whose vs who's confusion" },
       { pattern: /\bthere car\b/gi, penalty: 8, description: 'Their vs there confusion' },
       { pattern: /\bwhom\b/gi, penalty: -5, description: 'Bonus for correct whom usage' },
     ]);
   }
 }
 ...
 private static async assessDescriptiveWriting(content: string): Promise<number> {
   ...
   const descriptiveCount = words.filter((word) => adjectives.includes(word.toLowerCase())).length;
 ...
 private static assessCauseEffectLogic(story: string): number {
   ...
   const hasExplicitCausation = causeEffectWords.some((word) => story.toLowerCase().includes(word));
 ...
 private static calculateTextComplexity(content: string): number {
   ...
   const complexWords = 
     words.filter((word) => this.countSyllables(word) >= 3).length / words.length;
 ...
 private static analyzeComplexityForAge(text: string, age: number = 10): number {
   ...
   const actualComplexity =
     avgWordsPerSentence / 20 + avgSyllablesPerWord / 3 + complexWords / words.length;
 ...
 private static getParagraphStructure(paragraph: string) {
   ...
   return {
     ...
     startsWithTransition:
       /^(however|furthermore|moreover|additionally|consequently)/i.test(paragraph.trim()),
     endsWithConclusion:
       /(therefore|thus|in conclusion|finally).*[.!?]$/i.test(paragraph.trim()),
   };
 }
 ...
 private static createLinguisticFingerprint(content: string) {
   ...
   const complexSentences =
     sentences.filter((s) => s.includes(',') && s.includes('and')).length / sentences.length;
 ...
 private static analyzeContentAuthenticity(content: string, metadata?: any) {
   ...
   const authenticityScores = {
     ...
     ageAppropriateness: this.analyzeAgeAppropriatenessForPlagiarism(content, metadata?.childAge),
 ...
 private static calculateSentenceComplexity(text: string): number {
   ...
   const complexSentences = sentences.filter(
     (s) => s.includes(',') && (s.includes('because') || s.includes('although') || s.includes('while'))
   ).length;
 ...
 private static analyzeAgeAppropriatenessForPlagiarism(content: string, age: number = 10): number {
   ...
   const stats = await StorySession.aggregate([
     ...
     },
     {
       $group: {
         ...
         highRiskCount: {
           $sum: {
             $cond: [{ $in: ['$assessment.integrityRisk', ['high', 'critical']] }, 1, 0],
           },
         },
         aiDetectedCount: {
           $sum: {
