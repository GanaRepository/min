...
export class AdvancedPlagiarismDetector {
  ...
  static async checkPlagiarism(
    ...
    const { recommendations, educationalFeedback } =
      this.generateEducationalResponse(violations, overallScore, metadata);
  ...
  private static analyzeContentAuthenticity(content: string, metadata?: any) {
    ...
    const authenticityScores = {
      ...
      ageAppropriateness: this.analyzeAgeAppropriateness(
        content,
        metadata?.childAge
      ),
  ...
  private static analyzeAgeAppropriateness(
    content: string,
    age: number = 10
  ): number {
