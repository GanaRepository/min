...
export class PDFGenerator {
  ...
  private addHeader(story: StoryData) {
    ...
    this.doc.text(story.title, this.pageWidth / 2, this.currentY, {
      align: 'center',
    });
    this.currentY += 15;
    ...
    this.doc.text(`By ${story.authorName}`, this.pageWidth / 2, this.currentY, {
      align: 'center',
    });
    this.currentY += 10;
  ...
  private addStoryInfo(story: StoryData) {
    ...
    elements.forEach((element, index) => {
      const x =
        index % 2 === 0 ? this.margin : this.margin + columnWidth + this.margin;
    ...
    if (story.scores) {
      this.currentY += 8;
      this.doc.text(
        `Scores: Grammar ${story.scores.grammar}% • Creativity ${story.scores.creativity}% • Overall ${story.scores.overall}%`,
        this.margin,
        this.currentY
      );
    }
