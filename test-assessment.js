...
const testAssessment = async () => {
  try {
    ...
    const response = await fetch(
      'http://localhost:3001/api/stories/assessment/' + sessionId,
      {
        method: 'POST',
        headers: {
...
console.log(
  'Test script loaded. Assessment should now work with human-first approach.'
);
console.log('✅ Stories will ALWAYS get assessments');
console.log(
  '🏷️ Stories with integrity concerns will be TAGGED for mentor review'
);
console.log(
  '👨‍🏫 Mentors and admins can review flagged stories and provide guidance'
);
