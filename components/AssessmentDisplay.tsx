...
export default function AssessmentDisplay({ 
  ...
  const isAdvancedAssessment = assessment.assessmentVersion === '2.0' || 
  ...
  return (
    ...
    >
      ...
      {storyInfo && (
        <div className="bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg p-6 border border-blue-200">
          <div className="flex items-center justify-between">
            <div>
              <h2 className="text-xl font-semibold text-gray-900 mb-1">{storyInfo.title}</h2>
      ...
      {isAdvancedAssessment && (assessment.integrityAnalysis || assessment.plagiarismScore !== undefined) && (
        <div className="bg-white rounded-lg border p-6">
          <div className="flex items-center justify-between mb-4">
            <div className="flex items-center space-x-2">
              <Shield className="w-6 h-6 text-blue-600" />
              <h3 className="text-xl font-semibold text-gray-900">Content Integrity</h3>
            </div>
            <div className={`px-3 py-1 rounded-full border text-sm font-medium ${getRiskColor(
              assessment.integrityAnalysis?.integrityRisk || assessment.integrityRisk || 'low'
            )}`}>
              <div className="flex items-center space-x-1">
                {getRiskIcon(assessment.integrityAnalysis?.integrityRisk || assessment.integrityRisk || 'low')}
                <span className="capitalize">{assessment.integrityAnalysis?.integrityRisk || assessment.integrityRisk || 'Low'} Risk</span>
              </div>
            </div>
          </div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
            ...
            <div className="text-center p-4 bg-gray-50 rounded-lg">
              <div className={`text-3xl font-bold ${getScoreColor(
                assessment.integrityAnalysis?.originalityScore || assessment.plagiarismScore || 0
              )}`}>
                {assessment.integrityAnalysis?.originalityScore || assessment.plagiarismScore || 0}%
              </div>
            ...
            <div className="text-center p-4 bg-gray-50 rounded-lg">
              <div className="flex items-center justify-center space-x-1 mb-1">
                ...
                <div className={`text-2xl font-bold ${getScoreColor(
                  assessment.integrityAnalysis?.plagiarismScore || assessment.plagiarismScore || 0
                )}`}>
                  {assessment.integrityAnalysis?.plagiarismScore || assessment.plagiarismScore || 0}%
                </div>
              </div>
              ...
              {assessment.integrityAnalysis?.plagiarismRiskLevel && (
                <div className={`text-xs mt-1 px-2 py-1 rounded ${getRiskColor(assessment.integrityAnalysis.plagiarismRiskLevel)}`}>
            ...
            <div className="text-center p-4 bg-gray-50 rounded-lg">
              <div className="flex items-center justify-center space-x-1 mb-1">
                ...
                <div className={`text-2xl font-bold ${getScoreColor(
                  assessment.integrityAnalysis?.aiDetectionScore || assessment.aiDetectionScore || 0
                )}`}>
                  {assessment.integrityAnalysis?.aiDetectionScore || assessment.aiDetectionScore || 0}%
                </div>
              </div>
              ...
              {assessment.integrityAnalysis?.aiDetectionLikelihood && (
                <div className={`text-xs mt-1 px-2 py-1 rounded ${
                  assessment.integrityAnalysis.aiDetectionLikelihood === 'very_low' || 
                  assessment.integrityAnalysis.aiDetectionLikelihood === 'low' 
                    ? 'text-green-600 bg-green-50' 
                    : assessment.integrityAnalysis.aiDetectionLikelihood === 'medium' 
                    ? 'text-yellow-600 bg-yellow-50'
                    : 'text-red-600 bg-red-50'
                }`}>
                  {assessment.integrityAnalysis.aiDetectionLikelihood.replace('_', ' ')} AI likelihood
                </div>
              )}
            </div>
          </div>
            ...
            >
              <div className="flex items-start space-x-3">
                <div className={`mt-0.5 ${
                  warning.type === 'plagiarism' ? 'text-red-600' : 'text-orange-600'
                }`}>
                  {warning.type === 'plagiarism' ? <Search className="w-5 h-5" /> : <Brain className="w-5 h-5" />}
                </div>
                <div className="flex-1">
                  <div className={`font-medium ${
                    warning.type === 'plagiarism' ? 'text-red-800' : 'text-orange-800'
                  }`}>
                    {warning.type === 'plagiarism' ? 'Potential Plagiarism' : 'Possible AI Content'}
                  </div>
                  <div className={`text-sm mt-1 ${
                    warning.type === 'plagiarism' ? 'text-red-700' : 'text-orange-700'
                  }`}>
                    {warning.message}
                  </div>
      ...
      <div className="bg-gradient-to-r from-green-50 to-blue-50 rounded-lg p-6 border border-green-200">
        <div className="text-center">
          ...
          <div className="text-gray-600 text-lg">{assessment.readingLevel} Level</div>
        </div>
      </div>
      ...
      <div className="bg-white rounded-lg border p-6">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-4">
          {[
            ...
            { label: 'Creativity', score: assessment.creativityScore, icon: Star },
      ...
      {assessment.recommendations && (
        <div className="bg-purple-50 rounded-lg p-6 border border-purple-200">
          ...
          {assessment.recommendations.practiceExercises && assessment.recommendations.practiceExercises.length > 0 && (
            <div>
              <h5 className="font-medium text-purple-800 mb-2">Practice Exercises:</h5>
              <ul className="space-y-1">
                {assessment.recommendations.practiceExercises.map((exercise, index) => (
                  <li key={index} className="flex items-start space-x-2">
                    <Target className="w-4 h-4 text-purple-600 mt-0.5 flex-shrink-0" />
                    <span className="text-purple-700 text-sm">{exercise}</span>
                  </li>
                ))}
              </ul>
            </div>
      ...
      <div className="bg-gradient-to-r from-pink-50 to-purple-50 rounded-lg p-6 border border-pink-200">
        ...
        <p className="text-pink-800 leading-relaxed">{assessment.educationalInsights}</p>
      </div>
      ...
      {onReassess && storyInfo && storyInfo.attemptsRemaining > 0 && (
        <div className="text-center">
          ...
          >
            <TrendingUp className="w-5 h-5" />
            <span>Request New Assessment ({storyInfo.attemptsRemaining} left)</span>
          </button>
      ...
      {isAdvancedAssessment && 
       (assessment.integrityAnalysis?.integrityRisk === 'low' || assessment.integrityRisk === 'low') && 
       warnings.length === 0 && (
        <motion.div
          initial={{ opacity: 0, scale: 0.95 }}
          animate={{ opacity: 1, scale: 1 }}
        ...
        >
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h4 className="text-lg font-semibold text-green-900">Excellent Original Work!</h4>
              <p className="text-green-800 mt-1">
                Your content is completely original and authentic. Outstanding creative writing!
              </p>
            </div>
          </div>
        </motion.div>
