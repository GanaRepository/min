  ...
  return (
    ...
    >
      ...
      <div className="space-y-6">
        {/* Active Provider */}
        <div>
          <span className="text-gray-400 text-sm block mb-2">Active Provider:</span>
          <p className={`font-medium text-lg ${
            providerInfo.active.includes('Google') ? 'text-green-400' :
            providerInfo.active.includes('OpenAI') ? 'text-blue-400' :
            providerInfo.active.includes('Anthropic') ? 'text-purple-400' :
            'text-red-400'
          }`}>
        ...
        {/* Available Providers */}
        <div>
          <span className="text-gray-400 text-sm block mb-3">Available Providers:</span>
                ...
                <div key={index} className="flex items-center justify-between bg-gray-700/30 rounded-lg p-3">
                  <div className="flex items-center space-x-3">
                    <span className={`w-3 h-3 rounded-full ${
                      provider.name === 'Google' ? 'bg-green-400' :
                      provider.name === 'OpenAI' ? 'bg-blue-400' :
                      'bg-purple-400'
                    }`} />
                    <div>
                  ...
                  <span className={`text-sm font-medium ${
                    provider.cost === 'FREE' ? 'text-green-400' : 'text-yellow-400'
                  }`}>
                    {provider.cost}
                  </span>
                </div>
        ...
        {providerInfo.recommendations.length > 0 && (
          <div>
            <span className="text-gray-400 text-sm block mb-3">💡 Recommendations:</span>
            <div className="space-y-2">
              {providerInfo.recommendations.map((rec, index) => (
                <p key={index} className="text-yellow-300 text-sm bg-yellow-500/10 rounded-lg p-2">
                  {rec}
                </p>
              ))}
            </div>
        ...
        <div className="bg-gray-700/20 rounded-lg p-4">
          <h4 className="text-white font-medium mb-3">💰 Cost Comparison (per 1M tokens):</h4>
          <div className="space-y-2 text-sm">
            <div className="flex justify-between">
              <span className="text-green-400">🆓 Google Gemini 1.5 Flash:</span>
            ...
            <div className="flex justify-between">
              <span className="text-purple-400">💎 Anthropic Claude 3.5 Haiku:</span>
