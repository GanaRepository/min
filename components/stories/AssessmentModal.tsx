...
export default function AssessmentModal({
  ...
  const getCategoryScore = (category: string): number => {
    ...
    switch (category) {
      case 'grammar':
        return assessment.grammarScore;
      case 'creativity':
        return assessment.creativityScore;
      case 'vocabulary':
        return assessment.vocabularyScore;
      case 'structure':
        return assessment.structureScore;
      case 'characterDevelopment':
        return assessment.characterDevelopmentScore;
      case 'plotDevelopment':
        return assessment.plotDevelopmentScore;
  ...
  const categoryScores = [
    ...
    (category) =>
      category.score > 0 || !isAdvancedAssessment || category.group === 'core'
  );
              ...
              <div>
                ...
                <div className="mb-6">
                  ...
                  <div className="grid grid-cols-2 lg:grid-cols-4 gap-4">
                    {categoryScores
                      .filter((cat) => cat.group === 'core')
                      .map(({ label, category, score, icon: Icon, group }) => (
                        <div
                          key={category}
                ...
                {isAdvancedAssessment &&
                  categoryScores.filter((cat) => cat.group === 'advanced')
                    .length > 0 && (
                    <div className="mb-6">
                      ...
                      <div className="grid grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-4">
                        {categoryScores
                          .filter((cat) => cat.group === 'advanced')
                          .map(
                            ({ label, category, score, icon: Icon, group }) => (
                              <div
                                key={category}
              ...
              <div>
                ...
                <div className="mb-6">
                  ...
                  <div className="space-y-4">
                    {categoryScores
                      .filter((cat) => cat.group === 'core')
                      .map(({ label, category, score, icon: Icon }) => (
                        <div
                          key={category}
