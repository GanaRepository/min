    ...
    >
      ...
      <div className="bg-white rounded-lg border p-6">
        <div className="flex items-center justify-between mb-4">
          ...
          <div className={`px-3 py-1 rounded-full border text-sm font-medium ${getRiskColor(results.riskLevel)}`}>
        ...
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          ...
          <div className="text-center p-4 bg-gray-50 rounded-lg">
            <div className={`text-3xl font-bold ${getScoreColor(results.integrityScore)}`}>
          ...
          {results.plagiarismScore !== undefined && (
            <div className="text-center p-4 bg-gray-50 rounded-lg">
              <div className="flex items-center justify-center space-x-1 mb-1">
                ...
                <div className={`text-2xl font-bold ${getScoreColor(results.plagiarismScore)}`}>
                  {results.plagiarismScore}%
                </div>
              </div>
              <div className="text-sm text-gray-600">Originality</div>
              {results.plagiarismRiskLevel && (
                <div className={`text-xs mt-1 px-2 py-1 rounded ${getRiskColor(results.plagiarismRiskLevel)}`}>
          ...
          {results.aiDetectionScore !== undefined && (
            <div className="text-center p-4 bg-gray-50 rounded-lg">
              <div className="flex items-center justify-center space-x-1 mb-1">
                ...
                <div className={`text-2xl font-bold ${getScoreColor(results.aiDetectionScore)}`}>
              ...
              {results.aiDetectionLikelihood && (
                <div className={`text-xs mt-1 px-2 py-1 rounded ${
                  results.aiDetectionLikelihood === 'very_low' || results.aiDetectionLikelihood === 'low' 
                    ? 'text-green-600 bg-green-50' 
                    : results.aiDetectionLikelihood === 'medium' 
                    ? 'text-yellow-600 bg-yellow-50'
                    : 'text-red-600 bg-red-50'
                }`}>
                  {getAiLikelihoodText(results.aiDetectionLikelihood)} AI likelihood
                </div>
              )}
            </div>
          )}
      ...
      {results.riskLevel === 'low' && warnings.length === 0 && (
        ...
        >
          <div className="flex items-center space-x-3">
            ...
            <div>
              <h4 className="text-lg font-semibold text-green-900">Excellent Work!</h4>
              <p className="text-green-800 mt-1">
                Your content appears to be original and authentic. Keep up the great creative writing!
              </p>
            </div>
          </div>
        </motion.div>
