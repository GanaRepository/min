...
export default function WritingInterface({
  currentInput = '', // Default to empty string
  onInputChange,
  onSubmit,
  isSubmitting,
  session,
  ...
  useEffect(() => {
    ...
    const validationResult = validateTurnInput(inputText, turnNumber);
    setValidation({
      isValid: validationResult.isValid,
      message: validationResult.message || '',
    });
  ...
  return (
    <div className="bg-gray-800/60 backdrop-blur-xl border border-gray-600/40 rounded-xl p-6">
      ...
      <div className="mb-4">
        <textarea
          value={currentInput || ''} // Default to empty string
          onChange={(e) => onInputChange(e.target.value)}
          onKeyDown={handleKeyPress}
          placeholder="Continue your amazing story here..."
          disabled={isSubmitting || session.status === 'completed'}
      ...
      {/* Add safety check for trim */}
      {!validation.isValid && currentInput && currentInput.trim() && (
        <motion.div
          initial={{ opacity: 0, y: -10 }}
          animate={{ opacity: 1, y: 0 }}
          className="mb-4 p-3 bg-yellow-500/20 border border-yellow-500/30 rounded-lg"
