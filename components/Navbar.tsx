...
// components/Navbar.tsx - UPDATED VERSION with Creative Contests Integration
'use client';
...
const Navbar = () => {
  ...
  // Check for active creative contests
  useEffect(() => {
    const checkActiveContests = async () => {
      try {
        const response = await fetch('/api/creative-contests/active-count');
        const data = await response.json();
        if (data.success) {
          setHasActiveContests(data.count > 0);
          setContestCount(data.count);
        }
      } catch (error) {
        console.error('Error checking active contests:', error);
      }
    };

    checkActiveContests();

    // Check every 5 minutes for new contests
    const interval = setInterval(checkActiveContests, 5 * 60 * 1000);
      ...
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div className="flex justify-between items-center h-20">
          ...
          <div className="hidden md:flex items-center space-x-8">
            ...
            {/* Dynamic Creative Contests menu - only shows when active contests exist */}
            {hasActiveContests && (
              <Link
                href="/creative-contests"
                className={`text-lg transition-all duration-300 hover:text-contact-teal flex items-center ${
                  pathname.startsWith('/creative-contests')
                    ? 'text-contact-teal border-b-2 border-contact-teal pb-1'
                    : isScrolled
                      ? 'text-gray-300 hover:text-contact-teal'
                      : 'text-white hover:text-contact-teal'
        ...
        {isMobileMenuOpen && (
          <div className="md:hidden bg-gray-900 border-t border-gray-700 shadow-lg">
            <div className="px-2 pt-2 pb-3 space-y-1">
              ...
              {/* Dynamic Creative Contests menu for mobile - only shows when active contests exist */}
              {hasActiveContests && (
                <Link
                  href="/creative-contests"
                  className={`block px-3 py-2 text-base rounded-md transition-colors duration-200 flex items-center justify-between ${
                    pathname.startsWith('/creative-contests')
                      ? 'text-contact-teal bg-contact-teal/20'
                      : 'text-gray-300 hover:text-contact-teal hover:bg-gray-800'
                  }`}
